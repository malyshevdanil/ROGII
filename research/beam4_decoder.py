...
MIN_SEG_LEN = 15.0
LOCAL_CAL_FT = None  # None = global (whole known zone); a number = only use the last N ft near the transition
...
def calibrate(kn, tw_tvt, tw_gr, gr_full, local_ft=None):
    """local_ft: if set, only use the LAST local_ft feet of the known zone for the affine fit
    (adaptive/local recalibration near the known->eval transition) instead of the whole known zone."""
    if local_ft is not None:
        md = kn["MD"].values.astype(float)
        cutoff = md[-1] - local_ft
        kn = kn[md >= cutoff]
...
def decode_well(wid, data_dir=TRAIN_DIR, beam_width=BEAM_WIDTH):
    ...
    cal = calibrate(kn, tw_tvt, tw_gr, gr_full, local_ft=LOCAL_CAL_FT)
