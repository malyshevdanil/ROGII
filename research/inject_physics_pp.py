...
ROOT = 'd:/ROGII'
SRC = f'{ROOT}/7.178_moreseeds192_warpblend.ipynb'
DST = f'{ROOT}/7.178_moreseeds192_warpblend_physicspp.ipynb'

nb = json.load(open(SRC, encoding='utf-8'))

pp_cell_src = '''# --- PHYSICS POST-PROCESS: robust IRLS projection + warm-up damping + light smoothing -----------------
...
# Insert AFTER the WARP-blend cell, BEFORE guarded_contact_override
warp_idx = None
for i, c in enumerate(nb['cells']):
    if c['cell_type'] == 'code' and 'WARP DECORRELATION BLEND' in ''.join(c['source']):
        warp_idx = i
        break
assert warp_idx is not None, "warp-blend cell not found"
insert_at = warp_idx + 1
nxt = ''.join(nb['cells'][insert_at]['source'])
assert 'def guarded_contact_override' in nxt, f"unexpected next cell:\n{nxt[:200]}"

nb['cells'].insert(insert_at, make_cell(pp_cell_src))
