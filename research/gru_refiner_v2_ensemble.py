"""v2: fix the overfitting seen in gru_refiner.py (best@ep6, holdout climbed 9.85->11.6+ by ep80) with
stronger regularization (dropout 0.25->0.4, weight decay 3e-4->1e-3, input noise augmentation) and train
a MULTI-SEED ensemble (N=6, matching Tucker Arrants' own "six-model bidirectional GRU ensemble" -- his
writeup explicitly notes seed/feature-family diversity was the mechanism, not any single model's depth).
Same n_val=160,seed=42 holdout split throughout (shared with WARP/H4, honest reuse).
"""
import numpy as np, pickle, torch, torch.nn as nn, torch.nn.functional as F, time
from gru_refiner import build_dataset, GRURefiner, DEV, SEQ_LEN

CFG = dict(d=96, layers=2, drop=0.40, wd=1e-3, lr=1.2e-3, epochs=40, bs=16, n_val=160, seed=42,
...
if __name__ == '__main__':
    ...
    for seed in range(CFG['n_models']):
        r = train_one(seed, DATA, TR, VA, tag='v2')
        results.append(r)

    print('\nper-seed best:', results)

    # ensemble: average all seeds' best-checkpoint predictions
    preds = {w: [] for w in VA}
    for seed in range(CFG['n_models']):
        net = GRURefiner(21, CFG['d'], CFG['layers'], CFG['drop']).to(DEV)
        net.load_state_dict(torch.load(f'gru_refiner_v2_seed{seed}.pt', map_location=DEV))
        net.eval()
        with torch.no_grad():
    ...
    print(f'\nENSEMBLE ({CFG["n_models"]} seeds) holdout RMSE: {ens_rmse:.4f}  vs sp45={base_va:.4f}  vs best-single={min(r[0] for r in results):.4f}')

    pickle.dump({w: np.mean(preds[w], axis=0) for w in VA}, open('gru_refiner_v2_ensemble_preds.pkl', 'wb'))
