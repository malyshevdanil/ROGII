...
def build_dataset_v8(sp45_lookup, n_cuts=2, seed=0):
    ...
    for wid in wids:
        ...
        disagree = np.stack([sp45 - pf_ancc, sp45 - pf_z, pf_ancc - pf_z], axis=1).astype(np.float32)
        cuts = [1.0]
        ...
        for ci, cut_frac in enumerate(cuts):
            ...
            n = Xev.shape[0]
            if n != n_ev: continue
            Xev_full = np.concatenate([Xev, disagree], axis=1)  # (n, 27)
...
class GRURefinerV8(GRURefinerV7):
    ...
    for fold, (tr_i, va_i) in enumerate(gkf.split(dummy_X, groups=group_ids)):
        ...
        for seed_i in range(CFG['n_models_per_fold']):
            ...
            preds, net, mean, std = train_one_fold(DATA, TR_keys, VA_keys, seed,
                                                     save_tag=f'gru_v8_fold{fold}_seed{seed_i}')
        ...
        pickle.dump(dict(mean=mean, std=std), open(f'gru_v8_norm_fold{fold}.pkl', 'wb'))

    pooled = np.sqrt(np.mean(np.concatenate([(all_oof[k] - DATA[k]['src_true'])**2 for k in all_oof])))
    print(f'\npooled OOF RMSE (v8, +disagreement feats): {pooled:.4f}')
    pickle.dump(all_oof, open('gru_v8_kfold_oof_preds.pkl', 'wb'))
    print(f'saved gru_v8_kfold_oof_preds.pkl, total time {time.time()-t0:.0f}s')
