"""Stage 3c: strengthen track6 by ensembling multiple GBM seeds per fold (reduces prediction noise --
Stage 3b used a single model per fold, and per-row tree predictions are inherently noisy; averaging
several seeds should smooth out uncorrelated noise while keeping the genuine residual signal)."""
...
if __name__ == '__main__':
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(X, y, groups)):
        ...
        for s in range(N_SEEDS):
            model = lgb.LGBMRegressor(
                ...
                reg_lambda=10.0, reg_alpha=1.0, random_state=fold * 100 + s, verbose=-1)
            model.fit(X[tr_idx], resid[tr_idx])
            preds += model.predict(X[va_idx])
        pred_resid = preds / N_SEEDS
        ...
        print(f'fold {fold}: gbm_ens={rmse_gbm:.4f}  pf_ancc={rmse_pfancc:.4f}  n_val={len(va_idx)}  {time.time()-t0:.0f}s', flush=True)

    pooled_gbm = np.sqrt(np.mean((oof_abs - y)**2))
    pooled_pfancc = np.sqrt(np.mean((X[:, pf_col] - y)**2))
    print(f'\npooled RMSE: gbm_ens={pooled_gbm:.4f}  pf_ancc={pooled_pfancc:.4f}')
    print(f'fold RMSEs gbm_ens: {fold_rmse_gbm}')
    ...
    pickle.dump(dict(oof_abs=oof_abs, y=y, groups=groups, WIDS=WIDS,
                ...
                open('track6_stage3c_oof.pkl', 'wb'))
    print('saved track6_stage3c_oof.pkl')
