...
CFG=dict(d=128, KSTEPS=60, EVSTEPS=360, TWGRID=256, drop=0.2, wd=3e-4, lr=1.2e-3, epochs=140, bs=24,
         maxstep=1.5, smooth=0.06, aug_caljit=0.12, aug_noise=0.07, aug_warp=0.35, aug_twjit=0.06,
         w_known=0.5, ema_decay=0.995, n_val=160, seed=42, tag='h4ema')
...
def build_well(wid):
    ...
    # H4: MULTI-SCALE shape features (raw GR is self-similar; multi-res shape + band-pass ~ Eagle Ford bedding is more distinctive)
    cs=pd.Series(caln); sm5=cs.rolling(5,center=True,min_periods=1).mean().values
    sm15=cs.rolling(15,center=True,min_periods=1).mean().values; sm41=cs.rolling(41,center=True,min_periods=1).mean().values
    dog1=sm5-sm15; dog2=sm15-sm41                              # band-pass (fine / bedding-scale oscillation)
    ...
    return dict(H=np.stack([R(caln),R(sm15),R(sm41),R(dog1),R(dog2),R(gradn),R(rstdn),R(dzdmd)]).astype(np.float32),
...
d3=np.concatenate([w['H'][7] for w in DATA]); m3,s3=d3.mean(),d3.std()+1e-6   # dzdmd now ch7
...
class WarpNet(nn.Module):
    def __init__(s,d,drop):
        super().__init__(); s.he=Enc(8,d,drop); s.te=Enc(1,d,drop)   # H4: 8 multi-scale channels
    ...
    def forward(s,H,G,last_tvt):
        ...
        dtvt=torch.tanh(s.head(x)[...,0])*CFG['maxstep']       # (B,L) per-step derivative
        tvt=torch.cumsum(dtvt,dim=1)                          # integrate
        tvt=tvt-tvt[:,KSTEPS-1:KSTEPS]+last_tvt[:,None]        # anchor boundary at last known TVT
        return tvt
...
def evaluate(mdl):
    ...
    with torch.no_grad():
        for i in range(0,len(VA),CFG['bs']):
            H,G,lt,tvt,ev=batch(VA[i:i+CFG['bs']]); pred=mdl(H,G,lt); e.append(((pred-tvt)[ev>0.5]).cpu().numpy())
    mdl.train(); return float(np.sqrt((np.concatenate(e)**2).mean()))
# H5-lite: EMA of weights (implicit ensemble) -> kills the epoch-to-epoch bounce, gives a stable lower best
import copy
ema=copy.deepcopy(net); [p.requires_grad_(False) for p in ema.parameters()]; edec=CFG.get('ema_decay',0.995)
print('training WARP+H4+EMA | params=%.0fk'%(sum(p.numel() for p in net.parameters())/1e3),flush=True)
...
for ep in range(CFG['epochs']):
    ...
    for i in range(0,len(TR),CFG['bs']):
        H,G,lt,tvt,ev=batch([TR[j] for j in order[i:i+CFG['bs']]],True); pred=net(H,G,lt)
    ...
    if (ep+1)%2==0 or ep==CFG['epochs']-1:
        ...
        if re<bestema[0]: bestema=(re,ep+1); torch.save(ema.state_dict(),'best_ema.pt')
        print('  ep%2d loss%.3f | raw %.3f (best %.3f) | EMA %.3f (best %.3f@%d) %.0fs'%(
            ep+1,tot/nb,r,best[0],re,bestema[0],bestema[1],time.time()-t0),flush=True)
print('DONE raw best %.3f | EMA best %.3f @ep%d (flat~15, line~6.6, WARP 11.28)'%(best[0],bestema[0],bestema[1]),flush=True)
