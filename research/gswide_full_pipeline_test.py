"""Same question as ir-anchor, for the other isolated-tracker win: does gs-widening (found to help the bare
pf_ancc, 14.49->13.03 at gs_mult~1.5) still help once blended into the already-best combo
(sp45 + WARP-blend(0.30) + physics-pp)? Given the ir-anchor result (isolated win, zero value once stacked),
this is a direct test of whether that same base-dependent pattern repeats a third time.
"""
...
def gswide_track(wid, seed):
    r = load_well_raw(wid)
    if r is None: return None
    hw, tw_tvt, tw_gr, kn, ev = r
    gs = _gr_sig(hw, tw_tvt, tw_gr) * GS_MULT
    ...
    _seed_jit(seed)
    pts = _pf_ancc_base(md_v, z_v, gr_v, gg, gmin, gst, gs, ls, ir, N_PARTICLES,
...
print('computing seed-averaged gs-widened PF track for all wells...')
...
for i, wid in enumerate(WIDS):
    ...
    tracks = []
    for sd in SEEDS:
        t = gswide_track(wid, sd)
        if t is not None and len(t) == len(px['true']):
            tracks.append(t)
    if tracks:
        gs_track[wid] = np.mean(tracks, axis=0)
    if (i+1) % 200 == 0:
        print(f'  {i+1}/{len(WIDS)}  {__import__("time").time()-t0:.0f}s')
print('done:', len(gs_track), f'{__import__("time").time()-t0:.0f}s total')
...
def combo_with_gs(wid, w):
    combo = best_combo_track(wid)
    if w <= 0 or wid not in gs_track:
        return combo
    return (1 - w) * combo + w * gs_track[wid]
...
WIDS_GS = [w for w in WIDS if w in gs_track]
print('\n=== full-set sweep: best-combo + gs-widened-PF blend ===')
print('baseline:', pooled(lambda wid: best_combo_track(wid), WIDS_GS))
for w in [0.0, 0.05, 0.1, 0.15, 0.2, 0.3, 0.4, 0.5]:
    v = pooled(lambda wid, w=w: combo_with_gs(wid, w), WIDS_GS)
    print(f'  gs_blend_weight={w:.2f}  pooled={v:.4f}')

print('\n=== 5-fold cross-fit ===')
rng = np.random.default_rng(13)
order = np.array(WIDS_GS); rng.shuffle(order)
...
def best_w_on(wl):
    best = (1e9, None)
    for w in W_GRID:
        v = pooled(lambda wid, w=w: combo_with_gs(wid, w), wl)
...
for i in range(5):
    ...
    v = pooled(lambda wid, w=w: combo_with_gs(wid, w), test_wl)
    gains.append(b - v)
    print(f'fold {i}: baseline={b:.4f}  gs_weight={w:.2f}  with_gs={v:.4f}  gain={b-v:+.4f}')
