...
MIN_SEG_LEN = 15.0
LEVEL_WEIGHT = 0.0   # STRIDE's "typewell reference term": penalize candidates whose calibrated-GR
                      # LEVEL (mean) disagrees with the typewell's local level -- pure NCC only scores
                      # SHAPE (mean-subtracted), so it's blind to this. Untested in our own PF; this is
                      # the first test of it INSIDE a joint discrete decode (per the memory note that
                      # the earlier negative result may have been specific to a continuous AR(1) PF).
...
def decode_well(wid, data_dir=TRAIN_DIR, beam_width=BEAM_WIDTH):
    ...
    gr_scale = max(float(np.std(tw_gr)), 1e-6)
        ...
        for p in beam:
            ...
            for L in LEN_GRID:
                ...
                for zq in DIP_QUANTILES:
                    ...
                    level_lp = 0.0
                    if LEVEL_WEIGHT > 0:
                        level_diff = (seg_gr[v].mean() - seg_tw[v].mean()) / gr_scale
                        level_lp = -0.5 * level_diff ** 2
                    score = EMISSION_WEIGHT * corr + len_lp + dip_lp + LEVEL_WEIGHT * level_lp
