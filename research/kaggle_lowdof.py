...
CFG=dict(d=128, KSTEPS=60, EVSTEPS=360, TWGRID=256, drop=0.2, wd=3e-4, lr=1.2e-3, epochs=140, bs=24,
         maxstep=1.5, smooth=0.06, K_dof=0, aug_caljit=0.12, aug_noise=0.07, aug_warp=0.35, aug_twjit=0.06,
         w_known=0.5, n_val=160, seed=42, tag='lowdof')
...
def build_well(wid):
    ...
    z_rel=(Z-float(last['Z']))                                # known exactly in eval -> carries the TVT wiggle
    return dict(H=np.stack([R(caln),R(gradn),R(rstdn),R(dzdmd)]).astype(np.float32),
                gn=((g_gr-gm)/gs).astype(np.float32), tvt=R(true), z_rel=R(z_rel).astype(np.float32),
                ev=evr, last_tvt=last_tvt)
...
class WarpNet(nn.Module):
    def __init__(s,d,drop):
        ...
        s.head[-1].weight.data*=0.01; s.head[-1].bias.data.zero_()   # start ~flat (dTVT~0)
    def forward(s,H,G,last_tvt,z_rel):
        h=s.he(H); t=s.te(G)                                  # (B,d,L),(B,d,TG)
        Q=s.q(h).transpose(1,2); K=s.k(t).transpose(1,2); V=s.vv(t).transpose(1,2)  # (B,L,d),(B,TG,d)
        att=torch.softmax(Q@K.transpose(1,2)*s.sc,dim=2)      # (B,L,TG) read the typewell ruler
        ctx=att@V                                             # (B,L,d)
        x=torch.cat([h.transpose(1,2),ctx],dim=2)             # (B,L,2d)
        dtvt=torch.tanh(s.head(x)[...,0])*CFG['maxstep']       # (B,L) per-step derivative (PLAIN WARP)
        tvt=torch.cumsum(dtvt,dim=1); tvt=tvt-tvt[:,KSTEPS-1:KSTEPS]+last_tvt[:,None]   # anchor at boundary
        return tvt                                            # z_rel unused in model; only for post-hoc low-pass test
...
def batch(ws,train=False):
    ...
    tvt=np.stack([w['tvt'] for w in ws]).copy(); ev=np.stack([w['ev'] for w in ws]); zr=np.stack([w['z_rel'] for w in ws]).copy()
    if train:
        ...
        # TIME-WARP eval region (H + target tvt + z_rel together, keeps TVT=surf-Z consistent)
    ...
    return (to(H),to(gn)[:,None],to(np.array([w['last_tvt'] for w in ws])),to(tvt),to(ev),to(zr))
def evaluate():
    net.eval(); e=[]
    with torch.no_grad():
        for i in range(0,len(VA),CFG['bs']):
            H,G,lt,tvt,ev,zr=batch(VA[i:i+CFG['bs']]); pred=net(H,G,lt,zr); e.append(((pred-tvt)[ev>0.5]).cpu().numpy())
    net.train(); return float(np.sqrt((np.concatenate(e)**2).mean()))
print('training WARP | params=%.0fk'%(sum(p.numel() for p in net.parameters())/1e3),flush=True)
t0=time.time(); best=(99,0); order=np.arange(len(TR))
for ep in range(CFG['epochs']):
    np.random.shuffle(order); tot=0;nb=0
    for i in range(0,len(TR),CFG['bs']):
        H,G,lt,tvt,ev,zr=batch([TR[j] for j in order[i:i+CFG['bs']]],True); pred=net(H,G,lt,zr)
        m=ev+CFG['w_known']*(1-ev)
        loss=(F.smooth_l1_loss(pred,tvt,reduction='none',beta=8.0)*m).sum()/m.sum()
        d2=pred[:,2:]-2*pred[:,1:-1]+pred[:,:-2]                       # penalize jagged warp -> kills the drift-blowups
        loss=loss+CFG['smooth']*(d2*d2*m[:,2:]).sum()/m[:,2:].sum()
...
# ===== WARP-LOWPASS TEST: does lowpass(WARP_TVT + Z) - Z beat raw WARP? (is WARP's error HF?) =====
net.load_state_dict(torch.load('best_warp.pt')); net.eval()
def lpn(a,k):
...
with torch.no_grad():
    for i in range(0,len(VA),CFG['bs']):
        ws=VA[i:i+CFG['bs']]; H,G,lt,tvt,ev,zr=batch(ws)
        pr=net(H,G,lt,zr).cpu().numpy(); tv=tvt.cpu().numpy(); zn=zr.cpu().numpy(); em=ev.cpu().numpy()>0.5
        for b in range(len(ws)):
            mb=em[b]; p=pr[b][mb]; y=tv[b][mb]; z=zn[b][mb]
            raw.append((p-y)**2)
            surf=p+z                                          # surface_rel (= TVT+Z - const)
            for k in corr: corr[k].append((lpn(surf,k)-z-y)**2)
def pm(s): return float(np.sqrt(np.mean(np.concatenate(s))))
print('\n===== WARP-LOWPASS TEST =====',flush=True)
print('  raw WARP (best-checkpoint) = %.3f'%pm(raw),flush=True)
for k in [31,61,121]:
    print('  lowpass(WARP+Z)-Z k=%3d  = %.3f'%(k,pm(corr[k])),flush=True)
print('  (if lowpass << raw -> WARP error is HF -> BREAKTHROUGH: build it end-to-end)',flush=True)
