...
from gru_refiner_v3_submission_ready import build_dataset_v3, CFG
...
VA = [WIDS[i] for i in idx[:CFG['n_val']]]

ens_preds = pickle.load(open('gru_refiner_v3_ensemble_preds.pkl', 'rb'))
...
for w in VA:
    px = proxy[w]
    per_well[w] = dict(gru=ens_preds[w], full_combo=full_combo_track(w), true=px['true'])

def pooled(key, wl):
    sq = [(per_well[w][key] - per_well[w]['true']) ** 2 for w in wl]
    return float(np.sqrt(np.mean(np.concatenate(sq))))

print('gru v3 ensemble solo:', pooled('gru', VA))
...
for w_test in [0.1, 0.15, 0.2, 0.25, 0.3]:
    sq_cand = {w: (blend(w, w_test) - per_well[w]['true']) ** 2 for w in VA}
    mean_gain, lo, hi, frac_pos = paired_bootstrap_gain(sq_base, sq_cand, VA, n_boot=5000, seed=0)
